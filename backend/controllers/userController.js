...
import { cloudinaryDeleteArrayOfImage, cloudinaryDeleteImage, cloudinaryUploadImage } from '../cloudinary.js'
...
export let registerUser = async(req,res)=>{
  try{
    ...
    let token = jwt.sign({id:addUser._id,user:addUser.user,isAdmin:addUser.isAdmin},process.env.SECRET_KEY)
    await addUser.save()
    res.status(201).json({id:addUser._id,isAdmin:addUser.isAdmin,email:addUser.email,user:addUser.user,token:token})
  }catch(err){
    res.status(500).json(err.message)
  }
}
  ...
  try{
    ...
    if (bcrypt.compareSync(req.body.password, findEmail.password)) {
      const token = jwt.sign(
        {id:findEmail._id,user:findEmail.user,isAdmin:findEmail.isAdmin},
        process.env.SECRET_KEY,
      );
    res.status(201).json({id:findEmail._id,isAdmin:findEmail.isAdmin,email:findEmail.email,user:findEmail.user,token:token})
  }else{
